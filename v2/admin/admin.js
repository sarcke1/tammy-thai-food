...
async function loadOrders(){
  ...
  ordersEl.innerHTML=orders.map(order=>`<article class="order-card"><div class="order-top"><div><div class="order-number">Commande n°${order.order_number}</div><div class="order-meta">${escapeHtml(order.customer_first_name)} ${escapeHtml(order.customer_last_name)}<br>${escapeHtml(order.customer_phone||'')} · ${escapeHtml(order.customer_email||'')}<br>${new Date(order.created_at).toLocaleString('fr-FR')}</div></div><span class="badge">${escapeHtml(statusLabel(order.status))}</span></div><ul class="items">${(order.order_items||[]).map(item=>`<li><div><div class="item-name">${escapeHtml(item.product_name)} × ${item.quantity}</div><div class="item-detail">Piment : ${item.spice_level===0?'Sans piment':'Niveau '+item.spice_level}</div></div><span>${Number(item.unit_price).toLocaleString('fr-FR',{style:'currency',currency:'EUR'})}</span></li>`).join('')}</ul><div class="order-bottom"><span>Temps estimé : ${order.estimated_preparation_minutes||0} min</span><strong>${Number(order.total).toLocaleString('fr-FR',{style:'currency',currency:'EUR'})}</strong></div></article>`).join('');
}
