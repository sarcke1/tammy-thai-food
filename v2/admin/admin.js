...
async function activateAudio(){
  if(audioEnabled){
    audioEnabled=false;
    enableSoundButton.textContent='Activer le son';
    enableSoundButton.disabled=false;
    alertStatus.textContent='Alertes sonores désactivées';
    return;
  }
  enableSoundButton.disabled=true;
  enableSoundButton.textContent='Activation…';
  alertStatus.textContent='Activation du son…';
  try{
    const AudioCtor=window.AudioContext||window.webkitAudioContext;
    if(!AudioCtor)throw new Error('AudioContext non supporté par ce navigateur');
    audioContext=audioContext||new AudioCtor();
    if(audioContext.state==='suspended'){
      const resumePromise=audioContext.resume();
      await Promise.race([resumePromise,new Promise((_,reject)=>setTimeout(()=>reject(new Error('Le navigateur bloque l’accès audio')),1500))]);
    }
    if(audioContext.state!=='running')throw new Error('Le navigateur a refusé l’activation audio');
    audioEnabled=true;
    enableSoundButton.textContent='Désactiver le son';
    enableSoundButton.disabled=false;
    ...
    enableSoundButton.textContent='Activer le son';
    enableSoundButton.disabled=false;
    alertStatus.textContent='Son indisponible : '+error.message;
  }
}
...
async function playNotificationSound(repeat=true){
  ...
  try{
    if(audioContext.state==='suspended'){
      const resumePromise=audioContext.resume();
      await Promise.race([resumePromise,new Promise((_,reject)=>setTimeout(()=>reject(new Error('Audio suspendu')),1500))]);
    }
...
enableSoundButton.onclick=()=>{void activateAudio();};
