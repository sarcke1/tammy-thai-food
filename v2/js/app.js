...
async function loadProductsFromSupabase(){
  try {
    ...
    const remoteDishes = data.filter(product => product.name !== 'Nems').map(product => {
    ...
    dishes = [...remoteDishes, ...(localNems ? [localNems] : [])].sort((a,b) => (categoryOrder[a.category] || 99) - (categoryOrder[b.category] || 99) || (a.menuPosition || 999) - (b.menuPosition || 999));
    renderMenu(document.querySelector('.category-row .active')?.textContent.trim() || 'Tous');
  } catch(error) { console.error('Supabase unavailable — local menu kept:', error); }
}
...
function garnishRows(dish){ if(!dish.garnishOptions)return ''; const state=getState(dish); return `<div class="garnish-box"><div class="garnish-box-title"><span>Choisissez votre garniture</span><span>Quantité</span></div>${dish.garnishOptions.map(name=>`<div class="garnish-row"><span>Nems ${name.toLowerCase()}</span><div class="garnish-controls"><button type="button" data-garnish="${name}" data-garnish-delta="-1">−</button><b>${state.garnishes[name]||0}</b><button type="button" data-garnish="${name}" data-garnish-delta="1">+</button></div></div>`).join('')}</div>`; }
