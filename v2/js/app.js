...
function garnishRows(dish){
  if(!dish.garnishOptions)return '';
  const state=getState(dish);
  return `<div class="garnish-box"><div class="garnish-box-title">Choisissez votre garniture</div>${dish.garnishOptions.map(name=>`<div class="garnish-row"><span>${name}</span><div class="garnish-controls"><button type="button" data-garnish="${name}" data-garnish-delta="-1" aria-label="Diminuer ${name}">−</button><b>${state.garnishes[name]||0}</b><button type="button" data-garnish="${name}" data-garnish-delta="1" aria-label="Augmenter ${name}">+</button></div></div>`).join('')}</div>`;
}
