...
function getState(dish){ if(!cardStates.has(dish.id))cardStates.set(dish.id,{quantity:0,spices:[]}); return cardStates.get(dish.id); }
...
grid?.addEventListener('click',event=>{const card=event.target.closest('.dish-card');if(!card)return;const dish=dishes.find(d=>d.id===card.dataset.dishId);if(!dish)return;const state=getState(dish);const q=event.target.closest('[data-quantity-delta]');if(q){setQuantity(dish,state.quantity+Number(q.dataset.quantityDelta));renderMenu(document.querySelector('.category-row .active')?.textContent.trim()||'Tous');return;}const s=event.target.closest('[data-spice-delta]');if(s){const row=s.closest('[data-spice-row]');const i=Number(row.dataset.spiceRow);state.spices[i]=Math.max(0,Math.min(3,state.spices[i]+Number(s.dataset.spiceDelta)));renderMenu(document.querySelector('.category-row .active')?.textContent.trim()||'Tous');return;}if(event.target.closest('[data-action="add"]')){state.spices.slice(0,state.quantity).forEach(level=>addToCart(dish,dish.spicy?level:0));renderCart();cartPanel?.classList.add('open');}});
