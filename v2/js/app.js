...
function setProteinQuantity(dish,protein,quantity){ const state=getState(dish); const next=Math.max(0,Math.min(20,Number(quantity)||0)); state.proteinQuantities[protein]=next; state.quantity=Object.values(state.proteinQuantities).reduce((sum,value)=>sum+Number(value||0),0); if(state.quantity>20){ const overflow=state.quantity-20; state.proteinQuantities[protein]=Math.max(0,next-overflow); state.quantity=Object.values(state.proteinQuantities).reduce((sum,value)=>sum+Number(value||0),0); } syncSpices(state); if(state.quantity===0)state.chiliPacketQty=0; else state.chiliPacketQty=Math.min(1,state.chiliPacketQty); }
...
function chiliPacketSelector(dish){ if(!dish.chiliPacketAvailable)return ''; const state=getState(dish); return '<div class="spice-box chili-packet-box"><div class="spice-box-title">Supplément</div><div class="spice-row"><span>Sachet de piment supplémentaire</span><div class="spice-row-controls"><button type="button" data-chili-delta="-1" aria-label="Retirer le sachet de piment">−</button><b>'+(state.chiliPacketQty?'+ 0,50 €':'0 €')+'</b><button type="button" data-chili-delta="1" aria-label="Ajouter un sachet de piment">+</button></div></div></div>'; }
...
grid?.addEventListener('click',event=>{
  ...
  const cq=event.target.closest('[data-chili-delta]');
  if(cq){state.chiliPacketQty=Math.max(0,Math.min(1,state.chiliPacketQty+Number(cq.dataset.chiliDelta)));renderMenu(document.querySelector('.category-row .active')?.textContent.trim()||'Tous');return;}
  ...
  if(event.target.closest('[data-action="add"]')){
    let packetRemaining=state.chiliPacketQty;
    let spiceIndex=0;
    if(dish.proteinOptions?.length){
      dish.proteinOptions.forEach(protein=>{
        const quantity=Number(state.proteinQuantities?.[protein]||0);
        for(let i=0;i<quantity;i++){addToCart(dish,state.spices[spiceIndex]||0,protein,packetRemaining>0);if(packetRemaining>0)packetRemaining--;spiceIndex++;}
      });
    }else{
      for(let i=0;i<state.quantity;i++){addToCart(dish,state.spices[i]||0,state.protein,packetRemaining>0);if(packetRemaining>0)packetRemaining--;}
    }
