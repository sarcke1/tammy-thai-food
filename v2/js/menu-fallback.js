// V2 fallback renderer — keeps menu usable if app.js fails.
...
function renderFallback(refresh=false){
  ...
  grid.innerHTML=dishes.map(dish=>{
    ...
    return `<article class="dish-card" data-dish-id="${dish.id}"><div class="dish-photo" style="background-image:url('${dish.photo}');background-position:${dish.position||'center'};background-size:${dish.photo.endsWith('.webp')?'400% 300%':'cover'}"></div><div class="dish-card-body"><div class="dish-title-row"><h3>${dish.name}</h3><span class="price">${euro(dish.price)}</span></div><p>${dish.desc||''}</p>${isNems?`<div class="garnish-box"><div class="garnish-box-title">Choisissez votre garniture</div>${garnishNames.map(name=>`<div class="garnish-row"><span>Nems ${name.toLowerCase()} <small>Quantité</small></span><div class="quantity-controls"><button type="button" data-gminus="${name}">−</button><b>${garnishOf(dish.id,name)}</b><button type="button" data-gplus="${name}">+</button></div></div>`).join('')}</div>`:`<div class="quantity-row"><span>Quantité</span><div class="quantity-controls"><button type="button" data-minus>−</button><b>${quantity}</b><button type="button" data-plus>+</button></div></div>`}${dish.spicy&&quantity?`<div class="spice-box"><div class="spice-box-title">Piment</div><div class="spice-row"><span>${spiceLabel(spices.get(dish.id)||0)}</span><div class="spice-row-controls"><button type="button" data-spice-minus>−</button><b>${spiceLabel(spices.get(dish.id)||0)}</b><button type="button" data-spice-plus>+</button></div></div></div>`:''}<button class="button dish-action" type="button" data-add ${total?'':'disabled'}>Ajouter au panier</button></div></article>`;
  }).join('');
}
function refreshCart(){
  const panel=document.querySelector('#cart-panel'); const content=document.querySelector('#cart-content'); const count=document.querySelector('#cart-count');
  const items=getCart(); if(count)count.textContent=items.length;
  if(content)content.innerHTML=items.length?items.map(i=>`<div class="cart-line"><strong>${i.name}</strong><span>${euro(i.price)}</span></div>`).join(''):'<p class="empty-cart">Votre panier est vide.</p>';
  if(panel)panel.classList.add('open');
}

document.querySelector('#menu-grid')?.addEventListener('click',event=>{
 const card=event.target.closest('[data-dish-id]'); if(!card)return; const id=card.dataset.dishId; const dish=dishes.find(d=>d.id===id); if(!dish)return;
 ...
 if(event.target.closest('[data-add]')){if(id==='nems'){garnishNames.forEach(n=>{for(let i=0;i<garnishOf(id,n);i++)addToCart(dish,0,`Nems ${n.toLowerCase()}`);});garnishes.delete(id);}else{for(let i=0;i<quantityOf(id);i++)addToCart(dish,dish.spicy?(spices.get(id)||0):0);}quantities.set(id,0);renderFallback(true);refreshCart();}
});
document.querySelector('.cart-button')?.addEventListener('click',()=>{document.querySelector('#cart-panel')?.classList.toggle('open');refreshCart();});
document.querySelector('[data-cart-close]')?.addEventListener('click',()=>document.querySelector('#cart-panel')?.classList.remove('open'));
setTimeout(()=>{renderFallback(false);refreshCart();},800);
